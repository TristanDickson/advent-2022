...
for (const ins of instructions) {
  let srcStack = stacks[ins.src - 1];
  let dstStack = stacks[ins.dst - 1];
  let tower = srcStack.splice(srcStack.length - ins.num, srcStack.length);
  stacks[ins.dst - 1] = [...dstStack, ...tower];
}
